...
function sendAutoUpdateMessage(data: { [key: string]: any } | string) {
  win?.webContents.send('autoUpdate', data)
}

// 检查更新
autoUpdater.on('checking-for-update', () => {
  sendAutoUpdateMessage({
    eventType: 'checking-for-update',
    data: '正在检查更新...'
  })
})
...
autoUpdater.on('update-available', info => {
  sendAutoUpdateMessage({
    ...
    data: info // `发现新版本（${info.version}）～，准备开始下载...`
  })
})
...
autoUpdater.on('update-not-available', info => {
  sendAutoUpdateMessage({
    eventType: 'update-not-available',
    data: info
  })
})
